...
public class HuffmanCoding
{
    ...
    public String getCompressedCharString() {
        int length = compressedString.length();
        int padding =  8 - (length % 8);
//        String paddingIntInStr = ;
        String paddingIntInStr = String.format("%8s", Integer.toBinaryString(padding)).replaceAll(" ", "0");  // 8-bit Integer

//        System.out.println("padding in Int: " + padding);
//        System.out.println("padding in Str: " + paddingIntInStr);
        String paddedCompressedString = String.format("%" + (padding + length) + "s", compressedString).replace(' ', '0');
        System.out.println("padded compressed string length: " + paddedCompressedString.length());
        String contentInBinary = paddingIntInStr + paddedCompressedString;
        length = contentInBinary.length();
        StringBuilder str = new StringBuilder( length / 8);
        System.out.println("FInal length" + length);
        for(int i = 0; i < length; i += 8) {
            String temp = contentInBinary.substring(i, i + 8);
            System.out.println("worked with" + i);
